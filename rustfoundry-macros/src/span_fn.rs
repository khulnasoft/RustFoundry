...
mod tests {
    ...
    fn expand_sync_fn() {
        ...
        let expected = code_str! {
            fn do_sync<>() -> io::Result<String> {
                let __span = ::rustfoundry::telemetry::tracing::span("sync_span");
                {
                    do_something_else();
    ...
    fn expand_sync_fn_const_span_name() {
        ...
        let expected = code_str! {
            fn do_sync<>() -> io::Result<String> {
                let __span = ::rustfoundry::telemetry::tracing::span(some::module::SYNC_SPAN);
                {
                    do_something_else();
    ...
    fn expand_async_fn() {
        ...
        let expected = code_str! {
            async fn do_async<>() -> io::Result<String> {
                ::rustfoundry::telemetry::tracing::span("async_span")
                    .into_context()
                    .apply(async move {{
                        do_something_else().await;
    ...
    fn expand_async_trait_fn() {
        ...
        let expected = code_str! {
            ...
            {
                Box::pin(async move {
                    ::rustfoundry::telemetry::tracing::span("async_trait_span")
                        .into_context()
                        .apply(async move {
                            if let ::core::option::Option::Some(__ret) = ::core::option::Option::None::<String> {
                                return __ret;
