...
async fn main() -> BootstrapResult<()> {
    ...
    let service_info = rustfoundry::service_info!();
...
fn sandbox_syscalls() -> BootstrapResult<()> {
    use rustfoundry::security::common_syscall_allow_lists::{
        ASYNC, NET_SOCKET_API, SERVICE_BASICS,
    };
    use rustfoundry::security::{allow_list, enable_syscall_sandboxing, ViolationAction};
